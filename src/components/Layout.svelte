...
<div class="flex flex-col h-screen">
  <!-- Fixed Header -->
  <header class="flex items-center justify-between bg-black text-white w-full h-[75.2px] px-4 py-4 box-border shadow z-50 fixed top-0 left-0">
  ...
  <div class="flex flex-1 overflow-hidden">
    <!-- Sidebar -->
    <aside class={`w-64 bg-black text-white p-4 shadow-lg fixed top-[75.2px] left-0 h-[calc(100vh-75.2px)] overflow-y-auto z-50 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 lg:translate-x-0 lg:block`}>
      <ul class="space-y-1">
    ...
    <!-- Main Content -->
    <main class="flex-1 bg-gray-100 p-8 overflow-auto ml-0 lg:ml-64 mt-[75.2px]">
      <slot />
    </main>
  </div>
</div>
